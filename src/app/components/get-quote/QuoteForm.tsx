  ...
  return (
    <div>
      ...
      <div className='max-w-[1440px] mx-auto h-[103px] md:h-[111px]'>
        <h1 className='md:text-[56px] text-[24px] leading-[30px] md:leading-[64px] font-semibold'>{data.heading}</h1>
        <p className='md:text-[20px] text-[16px] leading-[20px] mt-3 md:leading-[26px] font-normal'>
          {data.subheading}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className='max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 h-[2001px] md:h-[1004px] gap-6 mt-6'>
          <div className='flex bg-[#612042] rounded-[24px] items-center justify-center h-full overflow-y-auto'>
            <div className='w-[614px] h-[892px] p-4'>
          ...
          <div className='flex rounded-[24px] items-center justify-center h-full'>
            <div className='w-[614px] h-[892px] text-[#2E2E2E] flex flex-col justify-between gap-5 p-4'>
              <h2 className='text-[32px] leading-[60px] font-semibold'>Choose Your Booth & Add-ons</h2>

              <div className='flex flex-col gap-[12px] h-[232px] justify-between p-3'>
                <p className='md:text-[24px] text-[20px] font-semibold leading-[60px]'>Booth Type</p>
                {data.eventBoothOptions.map((booth) => (
                  <div key={booth.id} className='flex gap-5 items-center h-[25px]'>
                    ...
                    <p className='text-[16px]'>{booth.label}</p>
                  </div>
                ))}
              </div>

              <div className='flex flex-col gap-[12px] h-[269px] justify-between p-3'>
                <p className='md:text-[24px] font-semibold text-[20px] leading-[60px]'>Add-Ons</p>
                {data.addOns.map((addon) => (
                  <div key={addon.id} className='flex gap-5 items-center h-[25px]'>
                    ...
                    <p className='text-[16px]'>{addon.label}</p>
                  </div>
                ))}
              </div>

              <div className='flex flex-col h-[232px] justify-between p-3'>
                <p className='md:text-[24px] text-[20px] font-semibold leading-[20px] md:leading-[60px]'>{data.themeQuestion}</p>
                <textarea name="themeRequest" value={form.themeRequest} onChange={handleChange} placeholder='Let us know about your event theme, color scheme, or logo needs.' className='h-full w-full border border-[#BFBFBF] rounded-[8px] p-2' />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className='w-full px-4 mt-10'>
          <h2 className='text-[32px] leading-[60px] font-semibold mb-2'>{data.contactHeading}</h2>
          ...
          <div className='w-full max-w-[614px] h-[158px] mt-6'>
            <h3 className='text-[24px] leading-[60px] font-semibold'>Preferred Contact Method</h3>
